...
electron-packager . "${APP_NAME}" --platform=$PLATFORM --arch=$ARCH --version=1.6.2 --app-version=1.4.0 --asar --icon=assets/icon.icns --overwrite --out=./dist --ignore=${ignore_list}
...
if [ $PLATFORM = "darwin" ]; then
    ditto -rsrcFork ./dist/Electronic\ System-darwin-x64/Electronic\ System.app /Applications/Electronic\ System.app
    echo "$(tput setaf 3)App copied to /Applications. You can open Electronic System there or from Spotlight.$(tput sgr0)"
fi
